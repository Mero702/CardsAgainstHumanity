...
io.on('connection', (socket) => {
    socket.on('joinRoom', (room, username, callback) => {
        ...
        if (!game)
            return callback({ ok: false, isMaster: false, error: `No game with the id "${room}" found` });
    ...
    socket.on('startGame', (callback) => {
        ...
        let error = game.startGame((socketID, cards, card, role) => io.to(socketID).emit('updateCards', cards, card, role));
        if (error)
