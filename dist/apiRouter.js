...
const ajv = new Ajv();
const CustomDecksSchema = JSON.parse(readFileSync(path.join(global.__dirname, "/src/CustomDeckSchema.json"), "utf8"));
const validateCustomDecks = ajv.compile(CustomDecksSchema);
const decks = new DeckLoader(path.join(global.__dirname, "/decks/cah-cards-full.json"));
const router = Router();
