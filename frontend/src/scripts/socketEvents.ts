...
export default function () {
  ...
  GameStore.$state.socket.on(
    "updateCards",
    (cards: WhiteCard[], blackCard: BlackCard, role: PlayerRole) => {
      GameStore.$state.cards.whiteCards = cards.map((x) => {
        return { selected: false, order: 0, ...x }
      })
  ...
  GameStore.$state.socket.on("voting", (answer) => {
    ...
    console.log(answer)
    GameStore.$state.cards.votingAnswers = answer.map((x) => ({
      selected: false,
      ...x,
    }))
  })
  GameStore.$state.socket.on(
    "WinnerAnnouncement",
    (username: string, blackCard, cards: Answer[], winner) => {
